...
async function fetchPlaylist(name) {
   try {
      const res = await fetch(`https://krdrtradio.github.io/player/${name}.m3u`);
...
downloadBtn.onclick = () => {
   ...
   const fileUrl = `https://krdrtradio.github.io/player/${fileName}`;
...
function openStreamLink(streamUrl) {
   ...
   fetch("https://krdrtradio.github.io/player/json/openstreamlink.json")
      .then(res => res.json())
      .then(json => {
         // Zmiana na json.site zgodnie z Twoją sugestią
