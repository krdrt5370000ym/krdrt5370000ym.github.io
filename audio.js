...
playerid_name=event.target.options[event.target.selectedIndex].dataset.text;
playerid_local=event.target.options[event.target.selectedIndex].text;
playerid_text=playerid_name+playerid_local;
document.getElementById('playerid_texts').innerHTML = playerid_text;
const player = videojs('hls-example');
player.src({src:playerid_src,type:playerid_type});
player.play();
